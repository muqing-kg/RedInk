...
- 支持上传参考图片，保持品牌视觉风格
- 支持单张图片重新生成，不满意就换（成功后覆盖旧图并继承文件名，失败不改动）
...
采用前后端分离架构：前端 Vue 3 + TypeScript + Vite，后端 Python Flask。用户数据与配置统一在 `/data` 目录。
...
- `/data/output`：运行中间产物（安全清理）
- `/data/history`：历史记录图片（业务数据，谨慎清理）
...
1. 图片文件命名从 **1 开始**（封面 `1.png`），DB 中索引为 0，但对外展示统一从 1；单页再生成成功后**继承原文件名**并覆盖旧图，失败不改动。\n2. API 配额：GCP 试用账号建议关闭高并发模式。\n3. 生成过程：图片生成需要时间，请保持页面活跃。\n4. 浏览器缓存：图片 URL 自动附加时间戳参数，避免旧图缓存。

## 🧹 缓存与清理
- 前端缓存：`frontend/node_modules/.cache/`、`frontend/node_modules/.vite/`、`frontend/.vite/`、`frontend/dist/`、`.eslintcache`\n- 后端缓存：`__pycache__/`、`*.pyc`、`*.pyo`、`.pytest_cache/`、`.mypy_cache/`、`.ruff_cache/`\n- 运行中间产物：`data/output/`（安全清理）\n- 历史图片：`data/history/`（谨慎清理；建议仅清理过期或孤立文件）\n\n> Windows PowerShell（安全清理示例）：\n```\nRemove-Item -Recurse -Force frontend\\node_modules\\.cache, frontend\\node_modules\\.vite, frontend\\.vite, frontend\\dist -ErrorAction SilentlyContinue\nGet-ChildItem -Recurse -Include *.pyc,*.pyo | Remove-Item -Force\nGet-ChildItem -Recurse -Directory -Filter __pycache__ | Remove-Item -Recurse -Force\nRemove-Item -Recurse -Force .pytest_cache, .mypy_cache, .ruff_cache -ErrorAction SilentlyContinue\nRemove-Item -Recurse -Force data\\output -ErrorAction SilentlyContinue\n```\n\n更多清理说明见 `docs/CLEANUP.md`。
